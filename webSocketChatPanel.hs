...
videoServerApp state request = do
  ...
  case msg of _
                | not(prefix `T.isPrefixOf` msg) -> sendTextData ("Wrong announcement" :: T.Text)
...
talk clients client@(user, _) = flip catchWsError catchDisconnect $
                                forever $ do
                                  msg <- receiveData
                                  liftIO $ readMVar clients >>=  broadcast (user `mappend` ": " `mappend` msg) 
                                where
...
main = do
  ...
  runServer "127.0.0.1" 8888 $ videoServerApp clients
